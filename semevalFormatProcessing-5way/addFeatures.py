	...
	if (header==1):
		try:
			if(line.split(" ")[0]=="@ATTRIBUTE" and line.split(" ")[1]=="accuracy"):
				...
				fo.write("@ATTRIBUTE q_neg_word_match NUMERIC\n")
				fo.write("@ATTRIBUTE q_num_match NUMERIC\n")
				fo.write("@ATTRIBUTE q_synonym_match NUMERIC\n")
				fo.write("@ATTRIBUTE q_antonym_match NUMERIC\n")
				fo.write("@ATTRIBUTE q_wup_similarity NUMERIC\n")
		except IndexError:
			pass
		fo.write(line+"\n");
		continue;
		...
		question_text_tokens=nltk.word_tokenize(question_text);
		...
		question_text_pos=nltk.pos_tag(question_text_tokens);
		#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		feature_neg_match=0;
		for neg_word in neg_words:
			if neg_word in ans_text_tokens and neg_word in expected_ans_text_tokens:
					...
					feature_neg_match-=1;
		feature_neg_match/=len(ans_text_tokens);
		#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		feature_q_neg_match=0;
		for neg_word in neg_words:
			if neg_word in ans_text_tokens and neg_word in question_text_tokens:
					feature_q_neg_match+=1;
			elif neg_word in ans_text_tokens and neg_word not in question_text_tokens:
					feature_q_neg_match-=1;
			elif neg_word not in ans_text_tokens and neg_word in question_text_tokens:
					feature_q_neg_match-=1;
		feature_q_neg_match/=len(ans_text_tokens);
		#print l
		#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		feature_num_match=0;
		...
		for w_a in ans_text_pos_nums:
				...
				if cd_present==1:
					feature_num_match-=1;
		feature_num_match/=len(ans_text_tokens);
		#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		feature_q_num_match=0;
		ans_text_pos_nums=[words for (words,pos) in ans_text_pos if pos=='CD'];
		question_text_pos_nums=[words for (words,pos) in question_text_pos if pos=='CD'];
		for w_a in ans_text_pos_nums:
				cd_present=0;
				for w_e in question_text_pos_nums:
						cd_present=1;
						if w_e==w_a:
							feature_q_num_match+=1;
							cd_present=0;
							break;
				if cd_present==1:
					feature_q_num_match-=1;
		feature_q_num_match/=len(ans_text_tokens);
		...
		for (word,pos) in ans_text_pos:
			max_wup=0;
			word_l=lmtzr.lemmatize(word);
			for(word_e,pos_e) in question_text_pos:
				word_e_l=lmtzr.lemmatize(word_e);
				try:
					max_wup=max(max_wup,wn.synsets(word_e_l)[0].wup_similarity(wn.synsets(word_l)[0]));
				except IndexError:
					pass
			q_wup_cumulative+=max_wup;
			if pos in pos_list:
				word_synsets=wn.synsets(word_l);
				synonyms=list(set([lemma2.key.split('%')[0] for lemma2 in [t.lemmas[0] for t in word_synsets]]));
				antonyms=list(set([lemma2.key.split('%')[0] for lemma2 in [t.lemmas[0].antonyms()[0] for t in word_synsets if t.lemmas[0].antonyms()]]));
				for (word_e,pos_e) in question_text_pos:
					...
					if word_e_l in synonyms:
						#check whether not is there or not
						if question_text_tokens.index(word_e)!=0 and ans_text_tokens.index(word)!=0:
							if (question_text_tokens[question_text_tokens.index(word_e)-1] in neg_words and ans_text_tokens[ans_text_tokens.index(word)-1] in neg_words) or (question_text_tokens[question_text_tokens.index(word_e)-1] not in neg_words and ans_text_tokens[ans_text_tokens.index(word)-1] not in neg_words):
								q_synonym_match+=1;
							else: 
								q_synonym_match-=1;
						...
						if question_text_tokens.index(word_e)!=0 and ans_text_tokens.index(word)!=0:
							if (question_text_tokens[question_text_tokens.index(word_e)-1] in neg_words and ans_text_tokens[ans_text_tokens.index(word)-1] in neg_words) or (question_text_tokens[question_text_tokens.index(word_e)-1] not in neg_words and ans_text_tokens[ans_text_tokens.index(word)-1] not in neg_words):
								...
								q_antonym_match+=1;
		q_synonym_match=q_synonym_match/len(ans_text_tokens);
		q_antonym_match=q_antonym_match/len(ans_text_tokens);
		q_wup_cumulative=q_wup_cumulative/len(ans_text_tokens);
		#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		

		fo.write(id1+","+overlapMax+","+fMax+","+leskMax+","+cosineMax+","+qOverlap+","+qf+","+qlesk+","+qcosine+","+str(word_matching(ans_text,expected_ans_text))+","+str(feature_neg_match)+","+str(feature_num_match)+","+str(synonym_match)+","+str(antonym_match)+","+str(wup_cumulative)+","+str(feature_q_neg_match)+","+str(feature_q_num_match)+","+str(q_synonym_match)+","+str(q_antonym_match)+","+str(q_wup_cumulative)+","+accuracy+"\n");			
